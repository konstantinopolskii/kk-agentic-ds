...
/* Vue twin of 03-identity-hero-v2.html. Same DOM, kit components where
   ...
   one metric with a rich delta (nested <span>, KMetric.delta is plain
   string only), and the trailing CTA button (kit KButton always stamps
   type="button" on a <button> element; the static markup omits it). */
export default {
  render() {
    return h(KApp, { view: 'single' }, () => [
      h(KBook, { id: 'doc' }, () => [
        h(KBookSection, () => [
          ...
          h('p', { class: 't-title' }, 'Growth expert. 30K MAU app owner. $1M+ B2B cases. 18 mentees holding $700K ARR. Two cats.'),
        ]),

        h(KBookSection, { id: 'founder' }, () => [
          h('h1', { class: 't-poster' }, 'Founder'),
          h('p', { class: 't-title' }, 'Scaling a self-built budgeting app from 30K users toward 1M. Fully self-funded.'),
        ...
        h(KBookSection, { id: 'strategist' }, () => [
          ...
          h('p', { class: 't-title' }, 'Growth strategy for B2B products that already work and want to grow up.'),
          h('div', { class: 'metric-row' }, [
          ...
          h('p', { class: 't-caption t-muted' }, 'Strategy sprints, growth audits, and the numbers to defend both.'),
        ]),

        h(KBookSection, { id: 'mentor' }, () => [
          h('h2', { class: 't-display' }, 'Mentor'),
          h('p', { class: 't-title' }, 'Founders through the jump from first revenue to a business that holds.'),
          h('div', { class: 'metric-row' }, [
        ...
        h(KBookSection, { id: 'now' }, () => [
          ...
          h('p', { class: 't-body' }, 'Current work is design systems for agent pipelines: kit vocabulary an agent reads instead of inventing parts on every job. The kit ships 19 components on 7 foundations; two consulting clients run it inside their agent builds today.'),
          h(KCard, () => [
            h(KCardHeading, { title: 'Working sessions', subtitle: '90 minutes, one decision made before you leave.' }),
            h('button', { class: 'button button--primary t-subtitle' }, 'Book a working session'),
          ]),
          h('p', { class: 't-micro t-muted' }, 'Amsterdam · kk.consulting · the cats stay in every reference.'),
        ]),
      ]),
    ])
  },
