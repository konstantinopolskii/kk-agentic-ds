...
1. **The code** — `vars.css`, `style.css`, `packages/vue` (sfc source + rebuilt dist, committed together), or the skill file you're changing.
...
7. **Integration doc** at `docs/integration/<component>.md` — required when the change touches a component's consumer-facing API: component props, emits, slots, exported composable signatures from `packages/vue`, `data-*` attributes consumers set. (Legacy surface for the frozen static demos: CustomEvents and `KK.config` keys in `js/kit.js` — frozen, documented in each doc's legacy section, never extended.) A component earns an integration doc the first time it has a public API surface; once the doc exists, it must be kept current in every PR that touches that surface. Today: comment, modal, dropdown, toast, pagination, sidebar-nav.
