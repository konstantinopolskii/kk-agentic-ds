...
Markdown embedded inside a `data-md-src` article renders with heading levels shifted +1 by default. An author writes `#` for their file's top heading and the shell places it at the article's h2 rank — the page already carries its own h1 and the part heading already owns hero size, so a second hero inside the article collapses the hierarchy. A container opts out with `data-md-heading-offset="0"` when the markdown file is the page root, and bumps to `data-md-heading-offset="2"` when the article sits under a part label that already owns display rank — `#` then lands at t-title, one step below the shell's part heading. Levels 1–4 resolve to the kit heading classes (t-hero, t-display, t-title, t-subtitle). Level 5 and deeper demote to a paragraph of caption-bold text (`<p class="t-caption--bold">`) — the kit has no heading rank below subtitle, so the renderer steps `#####` source down to 16 px bold caption with a one-line console info. Authors who write `####` then `#####` see a clear step down, not two identical lines.

Weight step at the bottom of the heading stack. `t-subtitle` renders at Medium 500, not bold, so the rank step against `t-caption--bold` (the demotion target for `#####`) reads as a weight drop rather than a 2-pixel size drop. At 18 px Medium vs 16 px Bold the step is visible under the 0.2-second bar; at 18 px Bold vs 16 px Bold (the prior state) the two ranks read as peers. Every other heading rank (hero, display, title) stays at Bold 700.
