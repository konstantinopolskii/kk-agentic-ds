...
1. The code. `vars.css`, `style.css`, `packages/vue` (sfc source plus rebuilt dist, committed together), or the skill file being changed. `js/kit.js` and `js/md.js` are frozen legacy — they serve the pre-2.0 static demos and take no new work; markdown rendering lives in `packages/vue/sfc/markdown.ts` and gates against `js/md.js` via `md_check.mjs`.
