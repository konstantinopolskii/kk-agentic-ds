...
4. **Foundations counts**: concrete counts are the **default**. "Nine colors," "seven type sizes," "three weights" stay in manifesto prose because the numbers teach the discipline viscerally. Abstract phrasing ("a bounded palette") is the **edge-case** reserved for claims that are genuinely fragile or hard to keep accurate — where the count changes often or is easily miscounted. Every abstract claim in manifesto prose must be flagged as "abstract by intent" so future authors don't mistake it for laziness. Radii specifically: the old drift was a wrong count, not fragility — fix the count concrete.
...
## Resolved round 3 — user rulings after Phase 2a audit

1. **components.md retirement path**: **(b) thin re-export stub for one minor cycle**. Ship markdown-as-source as **1.3.0** — the new renderer lands, manifesto absorbs, `components.md` becomes a short file that points its readers at the relevant `manifesto.md` sections. Every current consumer skill keeps reading `components.md` successfully through 1.3.0. Phase 4 of this initiative closes out at 1.3.0. A follow-up release bumps to **2.0.0** to remove the stub entirely. This buys a clean upgrade path and keeps the 12 skill canon-loads working while they migrate one at a time.
2. **Foundations counts posture**: concrete is the default per Resolved round 2 §4 (revised). Abstract stays as a flagged exception for fragile counts. Phase 2b executor keeps concrete counts where the number is stable.
3. **Signoff stats cardinality**: `components.md § Signoff` updates from "always two `.stat` children" to "two or four `.stat` children — both shapes are valid." No forced count. `style.css` already grids into three columns with the last intentionally empty when two stats ship; four stats fill the grid differently. Phase 2b executor relaxes the canon rule and keeps the three-column grid; fundamental's current two-stat demo stays valid.
