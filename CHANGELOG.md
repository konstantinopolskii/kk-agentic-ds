...
- `index.html` reshelled as a markdown-rendered canon doc (Phase 3). Line count 767 → 359 (53% reduction). Body prose now lives in `manifesto.md` and renders at runtime via `<div id="manifesto-mount" data-md-src="./skills/kk-design-system/manifesto.md" data-md-heading-offset="0">`. Shell keeps six pointer cards (Components, Patterns, Pipeline, Foundations, Practice, Distribution) wired to `demos/fundamental--accepted/`, `patterns.html`, `pipeline.md`, `manifesto.md`, and `kk-ds-maintainer/SKILL.md`. Signoff stats updated: "12 top-level sections across Meanings, Matter, Protocols" + "4 radii: 12, 16, 24, and 9999 for pills" replace the old "49 sections" and "2 radii" counts, both drifted at phase 2a. Inline post-render script listens for `kk:md-rendered`, wraps each top-level `##` + following siblings into its own `<article class="doc__section" id="<slug>">`, then calls `KK.refresh()` so scroll-spy observes the new sections and the sidebar TOC tracks them. Sidebar TOC rewired to seven hand-curated nav groups: Inventory, Distribution, Meanings, Principles, Matter, Protocols, Surfaces. Offset 0 on the mount — markdown's `# Manifesto` owns the page h1; shell carries no competing hero. Zero CSS changes — existing `.doc__section > h2/h3/h4` typography rhythm rules apply directly to the wrapped sections. `documentation/2026-04-24-markdown-source/phase-3-rewire.md` — phase self-doc with before/after counts, architecture diff, offset-decision reasoning, scroll-spy re-registration approach, verification (screenshots + console capture), and two open questions on signoff stats + TOC grouping.
